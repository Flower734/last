...
public:
	...
	void set(unsigned int data)
	{
		...
		this->m_bits[index0] |= (1 << temp0); //????4λ???1
...
index& SVS_NEON(int* list, vector<index>& idx, int num) {
	...
	for (int i = 1; i < num; i++) {//num???б???
		int count = 0;//s?е????????????????+1??δ?????erase??????????????????+1
		int length = s.len;
		for (int j = 0; j < length; j++) {//s?б??е????????
			bool isExit = false;
			uint32_t length = ceil(idx[list[i]].len / 4) * 4;
			for (int m = idx[list[i]].len ; m < length; m++)
				idx[list[i]].arr[m] = 0;
			for (int t = 0; t < length; t+=4 ) {//??i???б?????????s?б??е???j]??????
			...
			if (isExit == false) {
				s.len--;
				s.arr.erase(s.arr.begin() + count);//?????s?е?j?????
			}
			else {
				count++;
			}
...
index& Zipper(int* list, vector<index>& idx, int num) {
	...
	for (int i = 1; i < num; i++) {
		...
		while (count < length && p2 < idx[list[i]].len) {//?????δ????????β?
...
index& Hash_S_NEON(int* list, vector<index>& idx, int num) {//????????????????
	s = idx[list[0]];
	for (int i = 1; i < num; i++) {//num???б???
		int count = 0;//s?е????????????????+1??δ?????erase??????????????????+1
		int length = s.len;
...
void compare(int* list, vector<index>& idx, int num)
{
	...
	for (int i = 1; i < num; i++)
	{
		...
		{
			uint32x4_t zero_ = vmovq_n_u32(0);//???????0????????
			uint32x4_t temp0 = vmovq_n_u32(s_.sec_index[j]);//??0?????????????????i???????????
			uint32x4_t temp1 = vld1q_u32(&bitmap[i].sec_index[j]);//??i????????j????????????4?????
			uint32x4_t result = vornq_u32(temp0, temp1);//??λ???
			uint32x4_t isZero = vceqq_u32(zero_, temp0);//???
